...
export const getUserWordById = async (id: UserWordId) => {
  ...
  const [u] = await db
    ...
    .from(userWords)
    .where(
      and(eq(userWords.id, userWordId), eq(userWords.userId, session?.user.id))
    )
    .leftJoin(words, eq(userWords.wordId, words.id));
  return { userWord: u };
};
...
export const getAmountOfWordsPractied = async (languageId: LanguageId) => {
  ...
  if (!session?.user.id) throw Error('User ID is required');
  const u = await db
    .select({ words: userWords.wordId })
    .from(userWords)
    .innerJoin(words, eq(userWords.wordId, words.id))
    .innerJoin(lessons, eq(lessons.id, words.lessonId))
    .where(
      and(
        eq(userWords.userId, session?.user.id),
        eq(lessons.languageId, languageId || 1)
      )
    )
