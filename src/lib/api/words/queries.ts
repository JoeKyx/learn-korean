...
export const getWords = async () => {
  ...
  const w = await db
    .select({ word: words })
    .from(words)
    .where(or(eq(words.userId, session.user.id), isNull(words.userId)));
  return { words: w };
};
...
export const getWordWithUserWords = async (id: WordId) => {
  ...
  const w = await db
    .select()
    .from(words)
    .leftJoin(userWords, eq(userWords.wordId, words.id))
    .where(
      and(
        eq(words.id, wordId),
        or(eq(words.userId, session.user.id), isNull(words.userId))
      )
    );
  const categories = await db
    ...
    .where(
      and(
        eq(wordUserCategories.wordId, wordId),
        eq(wordUserCategories.userId, session.user.id)
      )
    );
  const word = {
    ...w[0].words,
    userWords: w.map((row) => row.user_words),
...
export const getWordsByLessonId = async (lessonId: number | string) => {
  ...
  const w = await db
    ...
    .where(
      and(
        eq(words.lessonId, lessonIdNumber),
        or(eq(words.userId, session.user.id), isNull(words.userId))
      )
    );
  return { words: w };
