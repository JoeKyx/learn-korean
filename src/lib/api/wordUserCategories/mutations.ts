...
export const createWordUserCategorie = async (
  wordUserCategorie: NewWordUserCategorieParams
) => {
  const { session } = await getUserAuth();
  if (!session?.user.id) throw new Error('User not found');

  const newWordUserCategorie = insertWordUserCategorieSchema.parse({
    ...wordUserCategorie,
...
export const updateWordUserCategorie = async (
  id: WordUserCategorieId,
  wordUserCategorie: UpdateWordUserCategorieParams
) => {
  const { session } = await getUserAuth();
  if (!session?.user.id) throw new Error('User not found');

  const { id: wordUserCategorieId } = wordUserCategorieIdSchema.parse({ id });
  const newWordUserCategorie = updateWordUserCategorieSchema.parse({
    ...wordUserCategorie,
    userId: session.user.id,
  });
  try {
...
export const deleteWordUserCategoryByCategoryId = async ({
  wordId,
  wordcategorieId,
}: {
  wordId: WordId;
  wordcategorieId: WordCategorieId;
}) => {
  const { session } = await getUserAuth();
  if (!session?.user.id) {
    throw Error('User is not logged in');
  }
  const { id: parsedWordCategorieId } = wordCategorieIdSchema.parse({
    id: wordcategorieId,
  });
  ...
  try {
    ...
    await db
      ...
      .where(
        and(
          eq(wordUserCategories.wordcategorieId, parsedWordCategorieId),
          eq(wordUserCategories.userId, session.user.id),
          eq(wordUserCategories.wordId, parsedWordId)
        )
      );
    return { success: true };
  } catch (err) {
