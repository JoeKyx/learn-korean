...
export const createDatesStudie = async (datesStudie: NewDatesStudieParams) => {
  ...
  const newDatesStudie = insertDatesStudieSchema.parse({
    ...datesStudie,
...
export const updateDatesStudie = async (
  id: DatesStudieId,
  datesStudie: UpdateDatesStudieParams
) => {
  const { session } = await getUserAuth();
  if (!session?.user.id) throw new Error('User not found');

  const { id: datesStudieId } = datesStudieIdSchema.parse({ id });
  const newDatesStudie = updateDatesStudieSchema.parse({
    ...datesStudie,
    userId: session.user.id,
  });
  try {
