...
import { 
  ...
  userSettings} from "@/lib/db/schema/userSettings";
...
export const createUserSetting = async (userSetting: NewUserSettingParams) => {
  ...
  const newUserSetting = insertUserSettingSchema.parse({ ...userSetting, userId: session?.user.id });
  try {
...
export const updateUserSetting = async (userSetting: UpdateUserSettingParams) => {
  const { session } = await getUserAuth();
  if(!session?.user.id) throw new Error("User not logged in");
  const newUserSetting = updateUserSettingSchema.parse({ ...userSetting, userId: session?.user.id });
  try {
    await db
     .update(userSettings)
     .set(newUserSetting)
...
export const upsertUserSetting = async (userSetting: UpdateUserSettingParams) => {
