    ...
    return (
      <div
        className={`p-2 flex items-center justify-between w-full transition-colors duration-200 rounded-lg group
                ${
                  word.userId
                    ? 'hover:bg-primary-500 bg-primary-300  cursor-pointer'
                    : 'bg-primary-200'
                }`}
        key={word.id}
      >
        ...
        {word.userId ? (
          <>
            ...
            <p className='text-center hidden group-hover:block w-1/5'>
              Edit Word
            </p>
          </>
        ) : (
  ...
  return (
    <div className='mt-4'>
      <div className='flex flex-col gap-2'>
        {words.map((word) =>
          word.userId ? <UserWord word={word} /> : <Word word={word} />
        )}
      </div>
