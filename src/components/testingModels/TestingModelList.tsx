...
export default function TestingModelList({ testingModels }: { testingModels: CompleteTestingModel[] }) {
  ...
  return (
    <ul>
      {t.testingModels.map((testingModel) => (
        <TestingModel testingModel={testingModel} key={testingModel.testingModel.id} />
      ))}
    </ul>
  );
}

const TestingModel = ({ testingModel }: { testingModel: CompleteTestingModel }) => {
  return (
...
const EmptyState = () => {
  return (
    <div className="text-center">
      <h3 className="mt-2 text-sm font-semibold text-gray-900">No testing models</h3>
