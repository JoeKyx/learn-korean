...
const CategoryOverview: FC = () => {
  ...
  const [currentCategoryFilters, setCurrentCategoryFilters] = useState<WordCategorie[]>([]);
  const practiceContext = useWordPractice();

  const { currentWord, wordCategories } = practiceContext.state;
  ...
  const onCategoryClickHandler = (category: WordCategorie) => {
    ...
    practiceContext.dispatch({ type: "SET_FILTERS", payload: { categories: tmpCategoryFilters } })

  };

  function CategoryIcon({ category, isSelected }: { category: WordCategorie, isSelected: boolean }) {
    const IconComponent =
      icons.find((icon) => icon.name === category.name)?.icon ||
      QuestionMarkIcon;

    return <IconComponent key={category.id} size={24} style={
      isSelected
        ? { fill: category.color }
        : {}
    } />;
  ...
  return (
    <div className="flex gap-2">
      {wordCategories.map((category) => {
        const isSelected = currentCategoryFilters.some((currentCategory) => currentCategory.id === category.id);
        ...
        return (
          <div
            ...
            key={category.id}
          >
            <CategoryIcon key={category.id} category={category} isSelected={isSelected} />
          </div>
        );
      })}
    </div>
