...
type WordPracticeAction =
  ...
  | { type: 'SET_FILTERS'; payload: { levels?: number[]; categories?: WordCategorie[] } }
  | { type: 'SHUFFLE_WORDS' }
  | { type: 'ADD_CATEGORY_TO_WORD'; payload: { wordId: number; categoryId: WordCategorieId; userId: string } }
  | { type: 'REMOVE_CATEGORY_FROM_WORD'; payload: { wordId: number; categoryId: number } }
  | { type: 'SET_MODE'; payload: { mode: ModeType } }
  ;

const applyFilters = (words: PracticeWords, filters: { levels: number[]; categories: WordCategorie[] }): PracticeWords => {
  return words.filter(word => {
    const wordLevel = calculateWordLevel(word);
    const isLevelMatch = filters.levels.length === 0 || filters.levels.includes(wordLevel);
    const isCategoryMatch = filters.categories.length === 0 || word.categories.some(wordCat => filters.categories.some(category => category.id === wordCat.wordcategorieId));
  ...
  switch (action.type) {
    ...
    case 'SWIPE_WORD': {
      ...
      if (currentWord) {
        ...
        let newLevel = action.payload.success ? currentLevel + 1 : currentLevel - 1;
        ...
        const oldWordIndex = state.filteredWords.findIndex(word => word.id === currentWord.id);
        ...
        const nextWord = updatedFilteredWords[nextWordIndex >= updatedFilteredWords.length ? 0 : nextWordIndex];
        const noWords = updatedFilteredWords.length === 0;
        return { ...state, currentWord: nextWord, allWords: updatedWords, mode: calculateMode(nextWord), filteredWords: updatedFilteredWords, noWords: noWords };
      }
      return state;
    }
    case 'SET_FILTERS': {
      ...
      const filteredWords = state.allWords.filter(word => {
        const wordLevel = calculateWordLevel(word);
        const isLevelMatch = updatedFilters.levels.length === 0 || updatedFilters.levels.includes(wordLevel);
        const isCategoryMatch = updatedFilters.categories.length === 0 || word.categories.some(wordCat => updatedFilters.categories.some(category => category.id === wordCat.wordcategorieId));
        return isLevelMatch && isCategoryMatch;
      });
      let noWords = false;
      let nextCurrentWord = state.currentWord;
      if (filteredWords.length && !filteredWords.includes(state.currentWord as PracticeWord)) {
    ...
    case 'ADD_CATEGORY_TO_WORD': {
      ...
      const updatedCurrentWord = state.currentWord.categories.some(cat => cat.wordcategorieId === action.payload.categoryId) ? state.currentWord : {
        ...state.currentWord,
      ...
      const newCurrentWord = updatedFilteredWords.includes(state.currentWord as PracticeWord) ? updatedCurrentWord : (updatedFilteredWords[0] || null);
    ...
    case 'REMOVE_CATEGORY_FROM_WORD': {
      ...
      const updatedCurrentWord = {
        ...state.currentWord,
        categories: state.currentWord?.categories.filter(cat => cat.wordcategorieId !== action.payload.categoryId)
      };
      const updatedAllWords = state.allWords.map(word => {
        if (word.id === action.payload.wordId) {
          return updatedCurrentWord
      ...
      const newCurrentWord = updatedFilteredWords.includes(state.currentWord as PracticeWord) ? updatedCurrentWord : (updatedFilteredWords[0] || null);
      logger(newCurrentWord, 'newCurrentWord')
      const newMode = newCurrentWord?.id !== state.currentWord?.id && newCurrentWord ? calculateMode(newCurrentWord) : state.mode
      return {
        ...state,
        allWords: updatedAllWords,
        filteredWords: updatedFilteredWords,
...
export const WordPracticeProvider: React.FC<WordPracticeProviderProps> = ({ children, initialWords, availableCategories }) => {
...
export const useWordPractice = () => {
  ...
  if (context === undefined) {
    throw new Error('useWordPractice must be used within a WordPracticeProvider');
  }
  return context;
}