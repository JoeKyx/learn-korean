...
const LessonProgress: React.FC<LessonProgressProps> = ({ practiceWords }) => {
  ...
  const [lessonProgress, setLessonProgress] = useState(calculateLessonProgress(allWords));
  ...
  const skillLevel = lessonProgress ? (lessonProgress.reduce((acc, word) => acc + word.level - 1, 0) / (lessonProgress.length * 4)) * 100 : 0;
  ...
  return (
    <div>
      ...
      <div className="flex flex-col items-center">
        ...
        <div className="flex">
          <span className="ml-2">{skillLevel ? skillLevel.toFixed(0) : 0}%</span>
        </div>
      </div>
